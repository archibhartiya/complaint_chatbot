...
#         return
#     # elif intent == 'faq':
#     #     # Only FAQ if no complaint ID in query
#     #     possible_ids = re.findall(r'\b([a-zA-Z0-9]{6,})\b', user_query)
#     #     if not possible_ids:
#     #         try:
#     #             response = qa.invoke(user_query)   # updated
#     #             st.session_state.chat_history.append(("bot", response or "🤖 I couldn't find that in our support info."))
#     #         except Exception as e:
#     #             st.session_state.chat_history.append(("bot", f"⚠️ RAG error: {str(e)}"))
#     #     else:
#     #         # Already handled by fetch
#     #         return


#     # ✅ Extra fallback: detect complaint ID even if intent was not "fetch"
#     possible_ids = re.findall(r'\b([a-zA-Z0-9]{6,})\b', user_query)
#     if possible_ids:
#         for cid in possible_ids:
#             try:
...
#                 st.session_state.chat_history.append(("bot", f"❌ Error retrieving complaint: {str(e)}"))
#                 return
#         st.session_state.chat_history.append(("bot", "❌ I couldn’t find a complaint with that ID. Please check and try again."))
#         return

#     # Default fallback
#     st.session_state.chat_history.append(("bot", "🤖 I'm not sure how to respond to that. You can say 'hi' to start over or ask a question."))

# # Run only if there's user input
# if user_query:
#     handle_user_input(user_query)
...
def handle_user_input(user_query):
    ...
    if st.session_state.stage in ['name', 'phone', 'email', 'details']:
        intent = 'register'
    elif st.session_state.stage == 'fetch':
        intent = 'fetch'
    else:
        intent = classify_user_intent(user_query)
    ...
    if intent == 'register':
        ...
        # already in fetch stage, expect complaint ID
        possible_ids = re.findall(r'\b([a-zA-Z0-9]{6,})\b', user_query)
        if possible_ids:
            for cid in possible_ids:
                try:
...
- **Status:** {data.get('status', 'Pending')}"""
                        st.session_state.chat_history.append(("bot", reply))
                        st.session_state.stage = 'chat'   # reset after success
                        return
                except Exception as e:
                    st.session_state.chat_history.append(("bot", f"❌ Error retrieving complaint: {str(e)}"))
                    st.session_state.stage = 'chat'
        ...
        try:
            response = qa.invoke(user_query)
            if response:
                # If QA returns a dict with query/result
